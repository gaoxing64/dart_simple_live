...
Future<List<LiveSuperChatMessage>> getHuyaSuperChatMessageList(
    {required int lPid, bool first = false}) async {
  final BaseTarsHttp messageBoardClient = BaseTarsHttp(
    "http://wup.huya.com",
    "wupui",
    headers: HuyaRequestParams.requestHeaders,
  );
  var userId = HuyaUserId()..sHuYaUA = HuyaRequestParams.HYSDK_UA;
  ...
  if (first) {
    return messages.length > 10 ? messages.sublist(10) : messages;
  } else {
